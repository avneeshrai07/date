...
function Celebrate(props) {
  ...
  const fireworks_elements = props.userData.fireworks;
  useEffect(() => {
    ...
    class Firework {
      constructor(x, y, targetX, targetY, shade, offsprings) {
        ...
        this.shade = shade;
        this.history = [];
        this.heart = fireworks_elements[random(0, fireworks_elements.length - 1)];
      }
      ...
      update(delta) {
        ...
        if (this.history.length === 0) {
          ...
          for (let i = 0; i < this.history.length; i++) {
            ...
            ctx.font = `${i + 5}px Arial`;
            ctx.fillText(this.heart, point.x, point.y);
          }
        } else {
          ctx.font = '20px Arial';
